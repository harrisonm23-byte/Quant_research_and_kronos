...
def summarize(trades, setup, mechanic, window):
    ...
    return {
        ...
        "p10_ret": trades["ret"].quantile(0.10) if len(trades) else np.nan,
        "p05_ret": trades["ret"].quantile(0.05) if len(trades) else np.nan,
        "worst_ret": trades["ret"].min() if len(trades) else np.nan,
